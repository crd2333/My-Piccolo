...
//     return count;
// }
int TypeMeta::getFieldsList(std::vector<FieldAccessor>& out_fields) const {
    out_fields = m_fields; // 复制 m_fields 到输出参数
    return out_fields.size();
}

int TypeMeta::getMethodsList(std::vector<MethodAccessor>& out_methods) const { // 新的实现
    out_methods = m_methods; // 复制 m_methods 到输出参数
